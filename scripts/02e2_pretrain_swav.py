...
def main():
    ...
    if start_epoch == 0:
        log.write("epoch,loss,lr,time_s,embed_std,avg_cosine_sim,"
                  "prototype_entropy,prototype_occupancy\n")
    log.flush()
    ...
    _p(f"  Physical Sinkhorn batch: {cfg.ssl_training.batch_size}")
    ...
    for epoch in range(start_epoch, cfg.ssl_training.epochs):
        ...
        for i, b in enumerate(loader):
            loss = trainer.train_step(b[0], step_idx=i, total_steps=n_steps)
            losses.append(loss)
        scheduler.step()
        ...
        log.write(f"{epoch},{avg:.6f},{lr:.8f},{elapsed:.1f},"
                  ...
                  f"{metrics['prototype_entropy']:.6f},"
                  f"{metrics['prototype_occupancy']:.6f}\n")
        log.flush()
        if epoch % 10 == 0 or epoch < 5 or epoch == cfg.ssl_training.epochs - 1:
            _p(f"  Epoch {epoch:3d} | Loss: {avg:.4f} | LR: {lr:.6f} | {elapsed:.0f}s | "
               ...
               f"proto H: {metrics['prototype_entropy']:.3f} | "
               f"occupied: {metrics['prototype_occupancy']:.3f}")
